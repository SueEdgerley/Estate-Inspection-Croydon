-- Add question_key to analytics view (from template snapshot; fallback to question_id).
...
SELECT
  ...
  r.question_id,
  COALESCE(
    NULLIF(trim(COALESCE(r.question_key_raw, '')), ''),
    r.question_id
  ) AS question_key,
  r.question_text,
...
COMMENT ON VIEW v_graded_inspection_answers_analytics IS
  'Analytics validation: graded answers with template labels and rank. '
  'question_key comes from snapshot (Airtable Question Key); else question_id. '
  'inspection_summary_grading is convenience only; stored_grade_value is authoritative per question.';
