...
export function canViewInspectionFollowUpUpdates({ roleCtx, userEmail, inspection, identity = null }) {
  if (!inspection) return false
  ...
  if (roleMustScopeToOwnOperationalRecords(operationalRole, roleCtx?.clerkIsAdmin)) {
    if (identity) return identityOwnsInspection(identity, inspection.inspector_id)
    return userOwnsInspection(userEmail, inspection)
  }
  if (['housing_team_manager', 'esm'].includes(operationalRole)) return true
