...
function isHealthAndSafetyDuplicateFollowUpRow(blob) {
  return (
    blob.includes('provide a photo') ||
    blob === 'comment' ||
    blob.includes('comments') ||
    blob.includes('who does this need')
  )
}
...
export function applyEsmInspectionFormPatch(template) {
  ...
  for (const section of sections) {
    ...
    for (const question of questions) {
      ...
      if (healthAndSafetySection && isHealthAndSafetyDuplicateFollowUpRow(blob) && !isMainHealthAndSafetyIssueQuestion(blob)) {
        question.esm_hidden = true
        question.nv_hidden = true
        continue
      }

      if (fireSafetySection && isHealthAndSafetyDuplicateFollowUpRow(blob) && !isMainFireSafetyIssueQuestion(blob)) {
        question.esm_hidden = true
        question.nv_hidden = true
        continue
      }
