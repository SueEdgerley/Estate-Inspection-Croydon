...
import { parseTriggersIssueAnswerList } from '@/lib/issue-trigger-answer'
...
function ensureSecurityOfIntakeRoomsQuestion(section) {
  ...
  if (existing) {
    ...
    ensureEsmGradedCdIssueTrigger(existing)
    return
  }
...
function isYesNoQuestion(question) {
  ...
  return typeText.includes('yes_no') || typeText.includes('yes no') || typeText.includes('yesno') || typeText.includes('yes/no')
}
...
function isEsmGradedQuestionType(question) {
  const typeText = String(
    question?.question_type || question?.question_type_raw || question?.answer_mode || ''
  ).toLowerCase()
  return typeText.includes('grad')
}

function ensureEsmGradedCdIssueTrigger(question) {
  if (!question || isYesNoQuestion(question) || !isEsmGradedQuestionType(question)) return
  const existing = parseTriggersIssueAnswerList(question)
  if (existing && existing.length > 0) return
  question.triggers_issue_answer = 'C,D'
}
...
function forceEsmGradedPhotoQuestion(question) {
  ...
  ensureEsmGradedCdIssueTrigger(question)
}
...
function suppressEsmIssuePhotoCommentControls(question) {
  question.include_photo = false
  question.type_includes_photo = false
  question.require_photo_on_yes = false
  question.esm_comment_always = false
  question.esm_comment_on_photo = false
  question.esm_comment_label = null
  question.esm_comment_helper = null
}
  ...
  for (const section of sections) {
    ...
    for (const question of questions) {
      ...
      if ((healthAndSafetySection || fireSafetySection || groundsMaintenanceSection) && isEsmYesNoIssueQuestionByText(question)) {
        ...
        const normalizedSectionName = (healthAndSafetySection || fireSafetySection)
          ? 'health_and_fire_safety_combined'
          : normalizeText(`${section?.title || ''} ${section?.name || ''}`)
        const issueKey = `${normalizedSectionName}::${normalizeText(
          [question?.question_text, question?.label, question?.question_key].filter(Boolean).join(' ')
        )}`
      ...
      if (externalCleaningGraffitiRemoval) {
        ...
        question.esm_use_people_recipients = true
        question.esm_recipient_label = 'Issue recipient'
      ...
      if (blob.includes('abandon') && (blob.includes('car') || blob.includes('vehicle'))) {
        ...
        if (isStandaloneDetailField && !isMainAbandonedVehicleQuestion) {
          ...
          configureEsmYesActionQuestion(question, {
            behavior: 'abandoned_vehicle',
            category: 'parking_abandoned_vehicle',
            priority: 'high',
          })
