...
export function formatDateGb(isoOrDate) {
...
export async function tryGenerateAndStoreIssueJobCardPdf(sql, p) {
  ...
  try {
    const pdfBuffer = await buildIssueJobCardPdfBuffer({
      ...
      location,
      exactLocation:
        exactLocation != null && String(exactLocation).trim() ? exactLocation : location,
      dateRaised,
      dateSent: dateSent != null && String(dateSent).trim() ? dateSent : dateRaised,
      issueTitle,
      issueType: issueType != null ? issueType : issueTitle,
      issueDetail,
      description: description != null ? description : issueDetail,
      priority: priority != null ? priority : '—',
      assignedTeam,
      targetCompletionDate: targetCompletionDate != null ? targetCompletionDate : 'TBC',
