...
export function buildInspectionWhereConditions({
  ...
  const fallbackIds = Array.isArray(fallbackInspectorIds)
    ? fallbackInspectorIds.map((value) => String(value || '').trim().toLowerCase()).filter(Boolean)
    : []
  if (!adminFlag && Array.isArray(fallbackInspectorIds)) {
    if (fallbackIds.length) {
      where.push(fragment`lower(trim(coalesce(inspector_id, ''))) = ANY(${fallbackIds}::text[])`)
    } else {
      where.push(fragment`FALSE`)
    }
  } else if (!adminFlag && fallbackInspectorId) {
