...
export function buildInspectionWhereConditions({
  ...
  // Explicit column comparisons only — nesting sql`col` fragments breaks @vercel/postgres (→ syntax error near $1).
  const dateToEnd = dateTo ? `${dateTo} 23:59:59` : ''
  if (dateFrom || dateTo) {
    if (dateField === 'due_date') {
      if (dateFrom) where.push(sql`due_date >= ${dateFrom}`)
      if (dateTo) where.push(sql`due_date <= ${dateToEnd}`)
    } else if (dateField === 'created_at') {
      if (dateFrom) where.push(sql`created_at >= ${dateFrom}`)
      if (dateTo) where.push(sql`created_at <= ${dateToEnd}`)
    } else if (scope === 'completed') {
      if (dateFrom) where.push(sql`submitted_at >= ${dateFrom}`)
      if (dateTo) where.push(sql`submitted_at <= ${dateToEnd}`)
    } else {
      if (dateFrom) where.push(sql`COALESCE(submitted_at, created_at) >= ${dateFrom}`)
      if (dateTo) where.push(sql`COALESCE(submitted_at, created_at) <= ${dateToEnd}`)
    }
  }
