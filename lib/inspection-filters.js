...
export function buildInspectionWhereConditions({
  ...
  if (scope === 'completed') {
    where.push(fragment`status = 'submitted'`)
  } else if (scope === 'active') {
    where.push(fragment`status IS DISTINCT FROM 'submitted'`)
  }
  // scope === 'all': no status filter (for analytics denominators, etc.)
  ...
  if (adminFlag && inspectorS && inspectorS !== 'all') where.push(fragment`inspector_id = ${inspectorS}`)
