...
export function isEstateInspectionFormTemplate(template) {
  ...
  const estateId = typeof process !== 'undefined' && process.env?.ESTATE_INSPECTION_TEMPLATE_ID?.trim()
  ...
  const key = String(template.template_key ?? template['Template Key'] ?? '')
    ...
    .trim()
  const envKey =
    typeof process !== 'undefined' && process.env?.ESTATE_INSPECTION_TEMPLATE_KEY?.trim().toLowerCase()
