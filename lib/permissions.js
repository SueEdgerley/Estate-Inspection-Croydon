...
const VALID_APP_ROLES = new Set([APP_ROLES.ADMIN, APP_ROLES.EDITOR])
const LOWEST_APP_ROLE = APP_ROLES.EDITOR
...
function normalizeAppRole(rawRole) {
  ...
  return normalized && VALID_APP_ROLES.has(normalized) ? normalized : null
}
...
function resolveAdHocCreatePermission(airtableUser, appRole) {
  ...
  return { canCreateAdHocInspection: false, adHocPermissionSource: 'default.editor_blocked' }
}

function getClerkIdMatchField(airtableUser, clerkUserId) {
  const candidateFields = ['Users ID', 'Clerk User ID', 'User ID']
...
export async function resolveCurrentUserAccess() {
  ...
  if (!appRole) {
    ...
    console.warn('[Permissions] Invalid/blank appRole; defaulting to editor permissions', {
      clerkUserId: userId,
      email,
      rawAppRole,
      jobTitle,
  ...
  return {
    ...
    permissions: {
      ...
      canCreateAdHocInspection,
      canCreateScheduledInspection: appRole === APP_ROLES.EDITOR || appRole === APP_ROLES.ADMIN,
    },
  }
}

export function buildAccessDeniedResponse(access, options = {}) {
  const { requireDashboard = false, requireAdmin = false, requireAdHocCreate = false } = options
