/**
 * Optional synthetic caretaker template (dev / emergency only).
 * By default caretaker inspections use the Airtable template as configured.
 *
 * Enable replacement with: CARETAKER_USE_CANONICAL_TEMPLATE=1
 */
...
/** Grading row: comment + photo always visible (standard inspection pattern). */
function gradedConditionRow(secKey) {
  return {
    ...
    is_required: true,
    standard_inspection_condition_row: true,
...
/** Issue row: Yes → comment, photo, recipient (standard inspection pattern). */
function yesNoRecipientRow(secKey, label) {
  return {
    ...
    answer_mode: 'yes_no',
    is_required: true,
    standard_inspection_issue_row: true,
...
function buildAbandonedVehiclesSection() {
  ...
  return {
    ...
    questions: [
      gradedConditionRow(secKey),
      yesNoRecipientRow(secKey, 'Abandoned vehicle issue to report?'),
      {
        id: qid(secKey, 'q3_vehicle'),
        question_key: `${secKey}_q3_vehicle`,
        question_text: 'Vehicle details',
        question_type: 'long_text',
        is_required: false,
      },
      {
        id: qid(secKey, 'q4_location'),
        question_key: `${secKey}_q4_location`,
        question_text: 'Location',
        question_type: 'long_text',
        is_required: false,
      },
      {
        id: qid(secKey, 'q5_cost'),
        question_key: `${secKey}_q5_cost`,
      ...
      {
        id: qid(secKey, 'q6_officer'),
        question_key: `${secKey}_q6_officer`,
        question_text: 'Officer details',
        question_type: 'text',
        is_required: false,
      },
      {
        id: qid(secKey, 'q7_confirm'),
        question_key: `${secKey}_q7_confirm`,
...
/**
 * When true, Airtable sections for caretaker templates are replaced by the synthetic tree above.
 * Default is false — production uses Airtable sections and questions as authored.
 */
export function shouldReplaceCaretakerTemplateWithCanonical() {
  return process.env.CARETAKER_USE_CANONICAL_TEMPLATE === '1'
}
