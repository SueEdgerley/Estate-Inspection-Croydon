...
function buildAbandonedVehiclesSection(title = 'Abandoned vehicles') {
...
function sourceSectionTitleByOrder(sourceSections) {
  const byOrder = new Map()
  if (!Array.isArray(sourceSections)) return byOrder
  for (const section of sourceSections) {
    const order = Number(section?.sort_order ?? section?.section_order ?? section?.order ?? 0)
    const title = String(section?.title ?? section?.name ?? '').trim()
    if (order > 0 && title) byOrder.set(order, title)
  }
  return byOrder
}

function buildCanonicalSections(sourceSections = []) {
  const sourceTitles = sourceSectionTitleByOrder(sourceSections)
  const sections = []
  for (let i = 0; i < 30; i++) {
    const n = i + 1
    const title = sourceTitles.get(n) || `Section ${n}`
    if (n === 2) {
      sections.push(buildAbandonedVehiclesSection(title))
      continue
    }
    const secKey = `sec${n}`
...
export function replaceWithCanonicalCaretakerTemplate(template) {
  ...
  return {
    ...
    sections: buildCanonicalSections(template.sections),
  }
}
