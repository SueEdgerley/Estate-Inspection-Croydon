...
const PEST_CONTROL_RECIPIENT_OPTIONS = [
  'palma.muriel@croydon.gov.uk',
  'kamran.hye@croydon.gov.uk',
  'kimberley.morris@croydon.gov.uk',
]
    ...
    questions: [
      {
        ...
        kind: 'issue_yes_no',
        recipientOptions: PEST_CONTROL_RECIPIENT_OPTIONS,
        recipientOnYes: true,
        photoAlways: false,
      },
    ],
...
function shouldAlwaysShowPhoto(sectionKey, index, def) {
  ...
  if (sectionKey === 'internal_cleaning') return index >= 0 && index <= 3
  return false
}

function isSimplePhotoCapture(sectionKey, index, def) {
  if (!shouldAlwaysShowPhoto(sectionKey, index, def)) return false
  return sectionKey === 'external_cleaning' || (sectionKey === 'internal_cleaning' && index >= 0 && index <= 3)
}
...
function buildQuestion(sectionKey, question, index) {
  ...
  const base = {
    ...
    ...(def.recipientOptions
      ? {
          caretaker_recipient_options: def.recipientOptions,
