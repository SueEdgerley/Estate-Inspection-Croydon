...
export const CARETAKER_OTHER_INTERNAL_CLEANING_QUESTION =
  'Are there any other internal cleaning issues to report?'
...
const SECTION_DEFINITIONS = [
  {
    ...
    questions: [
      ...
      { text: CARETAKER_OTHER_INTERNAL_CLEANING_QUESTION, kind: 'issue_yes_no', photoAlways: false },
      'Noticeboards',
    ],
  },
  {
  ...
  },
  {
    ...
    questions: [
      {
        text: 'Is there a pest control problem?',
        kind: 'issue_yes_no',
        recipientOnYes: true,
        photoAlways: false,
      },
  ...
  {
    key: 'repairs',
    title: '7. Any repairs to report?',
    questions: [
      { text: 'Any repairs to report?', kind: 'issue_yes_no' },
    ],
  },
]
...
function shouldAlwaysShowPhoto(sectionKey, index, def) {
  if (def.photoAlways != null) return def.photoAlways === true
  if (def.kind === 'issue_yes_no') return false
  if (sectionKey === 'external_cleaning') return true
  if (sectionKey === 'internal_cleaning') return true
  return false
}

function isSimplePhotoCapture(sectionKey, index, def) {
  if (!shouldAlwaysShowPhoto(sectionKey, index, def)) return false
  return sectionKey === 'external_cleaning' || sectionKey === 'internal_cleaning'
}

function buildQuestion(sectionKey, question, index) {
  const def = normalizeQuestionDef(question)
  const issueFollowUpOnYes =
    (sectionKey === 'internal_cleaning' && def.kind === 'issue_yes_no') ||
    sectionKey === 'asb' ||
    sectionKey === 'health_and_safety' ||
    sectionKey === 'fire_safety' ||
    sectionKey === 'repairs' ||
    sectionKey === 'pest_control'
  const base = {
    ...
    caretaker_comment_on_photo: true,
    caretaker_photo_always: shouldAlwaysShowPhoto(sectionKey, index, def),
    caretaker_simple_photo_capture: isSimplePhotoCapture(sectionKey, index, def),
    create_action_on_no: false,
    require_comment_on_no: false,
    require_photo_on_no: false,
    ...(def.recipientOnYes || def.recipientAlways
      ? {
          caretaker_recipient_on_yes: def.recipientOnYes === true,
          caretaker_recipient_always: def.recipientAlways === true,
          esm_use_people_recipients: true,
        }
      : {}),
  }

  if (def.kind === 'issue_yes_no') {
    const isOtherInternalCleaning = sectionKey === 'internal_cleaning'
    return {
      ...base,
      ...
      ...(issueFollowUpOnYes
        ? {
            caretaker_comment_on_yes: true,
            caretaker_photo_on_yes: true,
            action_category: sectionKey,
            category: sectionKey,
          }
        : {}),
      ...(isOtherInternalCleaning
        ? {
            require_comment_on_yes: true,
            comment_required_when: 'on_yes',
            require_photo_on_yes: false,
          }
        : {}),
    }
  }
