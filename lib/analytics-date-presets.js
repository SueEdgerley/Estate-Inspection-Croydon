...
/** Human label for Analytics / future Monthly Report period presets. */
export function analyticsPresetLabel(preset) {
...
export function resolveAnalyticsPresetDates(searchParams) {
  ...
  // Future Monthly Report button: This month | Previous month. Same resolver as Analytics.
  if (preset === 'previous_month' || preset === 'last_month') {
    const start = new Date(Date.UTC(end.getUTCFullYear(), end.getUTCMonth() - 1, 1))
    const lastDay = new Date(Date.UTC(end.getUTCFullYear(), end.getUTCMonth(), 0))
    return { dateFrom: isoUtcDate(start), dateTo: isoUtcDate(lastDay), preset: 'previous_month' }
  }
