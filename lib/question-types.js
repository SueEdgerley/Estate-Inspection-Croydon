...
export function getEffectiveQuestionKind(question) {
  ...
  // Explicit yes/no from template always wins over graded-style heuristics (caretaker rows).
  const rawTypeEarly = question.question_type ?? question.answer_mode
  if (rawTypeEarly != null && rawTypeEarly !== '') {
    const n = normalizeQuestionType(String(rawTypeEarly))
    if (n === 'yes_no') return 'yes_no'
  }
