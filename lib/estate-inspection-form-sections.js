...
/** Estate “1. Internal Cleaning” — stable Q1–Q8 order and one row per canonical prompt (by id). */
function isEstateInternalCleaningSection(sec) {
  const t = `${sec?.title || ''} ${sec?.name || ''}`.toLowerCase()
  return t.includes('internal cleaning')
}

function questionBodyTextLower(q) {
  return `${q?.question_text || ''} ${q?.label || ''} ${q?.resident_wording || ''}`.toLowerCase()
}

/**
 * Maps each checklist line to display order 1–8. Patterns are ordered so the long “stairs / tiles”
 * row matches before the shorter “entrance halls and lobbies” line (both mention lobbies).
 */
