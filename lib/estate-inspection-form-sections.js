...
export function buildEstateInspectionFormSections(template) {
  ...
  const useAirtableLayoutOnly = isEstateInspectionFormV2Template(template)
  ...
  /** V2: keep Airtable section links only — do not push unlinked rows into the first section. */
  if (!useAirtableLayoutOnly && orphans.length > 0 && sections.length > 0) {
    sections = [
