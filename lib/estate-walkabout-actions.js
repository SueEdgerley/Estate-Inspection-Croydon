...
function isEstateWalkaboutItemQuestion(question) {
  ...
  return (
    ...
    question.id !== 'ew_it_bulk_refuse_removal'
  )
}
...
async function createEstateWalkaboutActionsFromInspectionItems(sql, opts) {
  ...
  for (const section of templateVersion.sections || []) {
    for (const q of section.questions || []) {
      ...
      const comment = String(answersMap[`${q.id}_comment`] || '').trim()
      const photoUrls = Array.isArray(photoMap[q.id]) ? photoMap[q.id] : []
      if (!comment && photoUrls.length === 0) continue

      try {
        const existing = await sql`
          SELECT id FROM actions
          WHERE inspection_id = ${inspectionId} AND question_id = ${q.id} AND status = 'open'
          LIMIT 1
        `
        if (existing.rows.length > 0) continue

        const residentMessage = comment || q.question_text || q.label || 'Issue raised from inspection'
        ...
        await sql`
          ...
          VALUES (
            ...
            ${category}, null, ${residentMessage}, ${residentMessage}, ${locationLine || null}, 'open',
            ${comment || null}, true, ${JSON.stringify(photoUrls)}
          )
        `
        ...
        const actionPlanPdf = await tryGenerateAndStoreIssueJobCardPdf(sql, {
          actionId,
          inspectionId,
          inspectionType: templateVersion?.name || 'Estate walkabout',
          blockEstate: locationLine || estateName || '',
          location: locationLine || estateName || '',
          exactLocation: locationLine || estateName || '',
        ...
        if (isRepairRelatedWalkaboutAction({ description: residentMessage, action_summary: residentMessage, photo_urls: photoUrls })) {
          try {
            if (!repairRecipients) repairRecipients = await findWalkaboutRepairRecipients(sql)
...
async function findWalkaboutRepairRecipients(sql) {
  let routedRows = []
  try {
    ...
    console.warn('[estate-walkabout-actions] routing rule lookup failed:', error?.message || error)
  }
  ...
  `
  const seen = new Set()
  return [...routedRows, ...(result.rows || [])].filter((row) => {
...
export async function createEstateWalkaboutActionsFromPayload(sql, opts) {
  ...
  return { created, warnings, actions, emailResults }
}
