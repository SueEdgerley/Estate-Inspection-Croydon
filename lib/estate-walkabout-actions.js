...
async function findWalkaboutRepairRecipients(sql) {
  ...
  try {
    const routed = await sql`
      ...
      JOIN people p ON (
        ...
        OR (r.assign_to_role IS NOT NULL AND p.role = r.assign_to_role)
      )
      WHERE r.active = true
  ...
  const result = await sql`
    ...
    WHERE COALESCE(active, true) = true
      ...
      AND trim(email) <> ''
      AND (
        lower(COALESCE(role, '')) LIKE '%housing%officer%'
        OR lower(COALESCE(role, '')) LIKE '%repairs%officer%'
        OR lower(COALESCE(role, '')) LIKE '%repair%'
      )
    ...
    try {
      ...
      if (result.ok) {
        ...
        await logWalkaboutEmail(sql, {
          ...
          questionId,
          emailTo: email,
          routing: `estate_walkabout_repair:${recipient.role || 'person'}`,
          status: 'sent',
        })
      } else {
