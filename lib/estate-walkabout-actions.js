...
import { normalizeActionPhotoUrls, mergeActionPhotoUrls } from '@/lib/action-photos'
...
/** Standard item Qs where Yes (not No) means a problem / follow-up. */
export const WALKABOUT_ISSUE_ON_YES_QIDS = new Set([
...
/**
 * Whether a standard Walkabout yes/no item answer should create an action.
 * Photo + comment alone never create an issue — the Yes/No answer must trigger.
 */
export function inspectionAnswerTriggersWalkaboutItemIssue(question, section, answer) {
...
function normalizeWalkaboutDedupeText(value) {
  return String(value || '')
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, ' ')
    .replace(/\s+/g, ' ')
...
/**
 * @deprecated Prefer stable idempotency via inspection_id + question_id.
 * Kept only for unit tests of phrase overlap heuristics — not used at create time.
 */
export function walkaboutActionTextLooksDuplicate(existingRows, candidateText) {
  const cand = normalizeWalkaboutDedupeText(candidateText)
  if (!cand || cand.length < 16) return false
  const candWords = cand.split(' ').filter(Boolean)
  for (const row of existingRows || []) {
    const blob = normalizeWalkaboutDedupeText(
      [row?.title, row?.description, row?.comment].filter(Boolean).join(' ')
    )
    if (!blob) continue
    if (blob.includes(cand) || cand.includes(blob)) return true
    if (candWords.length >= 4) {
      for (let i = 0; i <= candWords.length - 4; i += 1) {
        const window = candWords.slice(i, i + 4).join(' ')
        if (window.length >= 12 && blob.includes(window)) return true
      }
    }
  }
  return false
...
function collectPhotoUrlsFromExtras(extras) {
  ...
  return mergeActionPhotoUrls(extras.photoUrl, extras.photo_urls, extras.photoUrls)
}
...
async function createEstateWalkaboutActionsFromInspectionItems(sql, opts) {
  ...
  const actions = []
  let created = 0
  const itemAnswers = omitHiddenWalkaboutPrivateGardensOvergrown(answersMap) || {}

  for (const section of templateVersion.sections || []) {
    for (const q of section.questions || []) {
      if (!isEstateWalkaboutItemQuestion(q)) continue

      const answer = itemAnswers[q.id]
      if (answer === undefined || answer === null) continue
      if (
        q.id === EW_IT_PRIVATE_GARDENS_OVERGROWN_QID &&
        !isWalkaboutPrivateGardensOvergrownVisible(itemAnswers)
      ) {
        continue
      }
      ...
      const comment = String(
        isBulkRefuse ? itemAnswers.ew_it_bulk_refuse_comments || '' : itemAnswers[`${q.id}_comment`] || ''
      ).trim()
      const exactLocation = String(isBulkRefuse ? itemAnswers.ew_it_bulk_refuse_exact_location || '' : '').trim()
  ...
  if (!isWalkaboutPrivateGardensOvergrownVisible(itemAnswers)) {
    try {
      await sql`
        DELETE FROM actions
        WHERE inspection_id = ${inspectionId}
          AND question_id = ${EW_IT_PRIVATE_GARDENS_OVERGROWN_QID}
          AND auto_created = true
      `
    } catch (cleanupErr) {
      warnings.push(
        `Could not clear hidden garden overgrown action: ${cleanupErr?.message || String(cleanupErr)}`
      )
    }
  }
...
async function sendWalkaboutRepairActionEmail(sql, {
  ...
  const photos = normalizeActionPhotoUrls(item.photo_urls)
...
export async function createEstateWalkaboutActionsFromPayload(sql, opts) {
  ...
  // Standard checklist items first (idempotent by inspection_id + question_id).
  // Additional items use ew_chk_<item.id> — no soft text dedupe of unrelated issues.
  ...
  const actions = [...(itemResult.actions || [])]
  ...
  for (const item of items) {
    ...
    const photoUrls = normalizeActionPhotoUrls(item.photo_urls)
...
export async function createEstateWalkaboutActionsFromInspection(sql, opts) {
  ...
  // Single path: checklist additional items + standard item questions (idempotent by question_id).
  const answers = { ...answersMap, [ESTATE_WALKABOUT_CHECKLIST_QID]: raw }
