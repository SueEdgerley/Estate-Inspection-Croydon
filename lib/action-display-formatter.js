import { formatIssueReference } from '@/lib/issue-number'
...
const BLOB_URL_PATTERN = /blob:https?:\/\/\S+/gi
...
export function cleanActionDisplayText(value, { preserveLabels = false } = {}) {
  ...
  if (/^blob:https?:\/\//i.test(text)) return ''
...
function cleanInlineText(value) {
  const cleaned = String(value || '')
    ...
    .replace(/[ \t]{2,}/g, ' ')
    .trim()
  return EMPTY_PLACEHOLDERS.has(normalizeComparableText(cleaned)) ? '' : cleaned
}
...
export function categoryLabel(value) {
  ...
  if (raw.toLowerCase() === 'pest_control') return 'Pest Control'
...
function isPlaceholderPerson(value) {
  return normalizeComparableText(value) === 'inspector'
}

function firstPersonText(...values) {
  for (const value of values) {
    const cleaned = cleanActionDisplayText(value)
    if (cleaned && !isPlaceholderPerson(cleaned)) return cleaned
  }
  return ''
}
...
function isTemplateOnlyLocation(value, action) {
  const normalized = normalizeComparableText(value)
  if (!normalized) return false
  if (/^(estate walkabout|esm inspection form|caretaker inspection)(\s*[-–]\s*)?$/i.test(String(value || '').trim())) {
    return true
  }
  const templateNames = [
    action.inspection_template_name,
    ...
    .filter(Boolean)
  return templateNames.includes(normalized)
}

function isInternalLocationLabel(value) {
  const normalized = normalizeComparableText(value)
  return (
    INTERNAL_TOKEN_LABELS.has(normalized) ||
    /^esm(?:[_\s-]+[a-z0-9]+)+$/i.test(String(value || '').trim()) ||
    /^(action category|inspection id|photo references?|recipient)$/i.test(normalized)
  )
}

function firstCleanLocation(action, ...values) {
  for (const value of values) {
    const cleaned = cleanActionDisplayText(value)
    if (cleaned && !isTemplateOnlyLocation(cleaned, action) && !isInternalLocationLabel(cleaned)) return cleaned
  }
  return ''
}
...
export function buildActionDisplay(action = {}) {
  ...
  const inspectionTemplateName = firstCleanText(
    action.inspection_template_name,
    action.template_title,
    action.template_name,
    action.inspection_form_name,
    action.form_name,
    action.inspection_type,
    action.type
  )
  const estateBlockLocation = firstCleanLocation(
    action,
    [action.estate_name, action.block_name].filter(Boolean).join(' / '),
  ...
  const specificLocation = firstCleanLocation(
    action,
    action.location,
    action.inspection_location_label,
    action.inspection_address,
  ...
  const comment = firstCleanText(action.comment, parsed.comment)
  const completedBy = firstPersonText(
    ...
    action.submittedBy,
    action.created_by,
    action.current_user_name,
    action.current_user_email,
    action.inspection_user_email,
    action.user_email,
    parsed['submitted by'],
    action.inspector_name,
    action.inspection_inspector_email,
    action.inspector_email,
    action.inspection_inspector_id,
    action.inspector_id
  )
  ...
  return {
    ...
    completedBy,
    inspectorName: completedBy,
    estateBlockLocation,
    specificLocation,
    contextLocation: [estateBlockLocation, specificLocation].filter(Boolean).join(' - '),
    section,
    issue,
    rating,
    comment,
    location: estateBlockLocation,
    blockLocation: specificLocation,
    ...
    issueReference: formatIssueReference(action.issue_number),
