/**
 * Resolve a person for an issue using issue_routing_rules then people.job_title / direct id.
 * Falls back to people.role for legacy rows that predate job_title.
...
export async function resolveIssueRoutingRecipient(sql, params) {
  ...
  try {
    ...
    if (error?.code !== '42P01' && !/relation\s+"?issue_routing_rules"?\s+does not exist/i.test(String(error?.message || error))) {
      throw error
    }
    console.warn('[resolveIssueRoutingRecipient] issue_routing_rules table missing; continuing without routing rules')
  }
  ...
  const byRole = await sql`
    SELECT id, email, name FROM people
    WHERE (job_title = ${role} OR role = ${role})
      AND COALESCE(active, true) = true
