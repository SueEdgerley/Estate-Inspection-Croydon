...
function gradedQ(id, questionText, sortOrder) {
  return {
    ...
    create_action_on_c: true,
    create_action_on_d: true,
    ...
    sections: [
      ...
      },
      {
        ...
        questions: [
          ynQ(
            'gm_s3_q1',
            'Is litter removal from communal areas, grassed areas and shrubs satisfactory?',
            1,
            'no'
          ),
        ],
...
export function applyGroundsMaintenanceTemplateToSnapshot(templateVersion) {
  ...
  const storedTextById = new Map()
  for (const section of templateVersion.sections || []) {
    for (const question of section.questions || []) {
      if (!question?.id) continue
      storedTextById.set(String(question.id), {
  ...
  const sections = canonical.sections.map((section) => ({
    ...
    questions: (section.questions || []).map((question) => {
      const stored = storedTextById.get(String(question.id))
      if (!stored) return question
      return {
        ...question,
        question_text: stored.question_text || question.question_text,
        label: stored.label || stored.question_text || question.label,
      }
    }),
  }))
  return {
