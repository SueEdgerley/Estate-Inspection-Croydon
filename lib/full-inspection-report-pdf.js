...
function buildPdfAction(row, questionMetaById, inspectionRow, locationLine, answerByQuestionId) {
  ...
  return {
    ...
    raisedBy: cleanActionDisplayText(inspectionRow.inspector_name) || '',
    photoUrls,
...
export async function buildFullInspectionReportPdfPayload(sqlFn, inspectionId, inspectionRow) {
  ...
  const openActions = (actionRows.rows || [])
    .map((row) => buildPdfAction(row, questionMetaById, inspectionRow, estateBlockLine, answerByQuestionId))
  ...
  for (const section of sections) {
    ...
    for (const question of section.questions || []) {
      ...
      const gradeValue = question.grading_scheme_name ? String(answerVal) : undefined
      const resultMode = resolvePdfResultMode(question, answerVal)
  ...
  for (const row of answersResult.rows || []) {
    ...
    fallbackBySection.get(sectionTitle).push({
      ...
      comment: comment || undefined,
      grade: gradeValue,
      resultMode: resolvePdfResultMode(meta?.question, answerVal),
      hasIssue: reportableIssueQuestionIds.has(qid),
    })
    includedQuestionIds.add(qid)
    const questionPhotos = photosByQuestionId[qid] || []
