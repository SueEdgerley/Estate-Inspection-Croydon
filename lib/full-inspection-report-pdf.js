...
function parseActionPhotoUrls(raw) {
  ...
  try {
    ...
    return raw.startsWith('http') ? [raw.trim()] : []
  }
}

function formatActionStatus(raw) {
  const status = String(raw || '').trim().toLowerCase().replace(/[_-]+/g, ' ')
  if (!status) return 'Open'
  if (status === 'in progress' || status === 'inprogress') return 'In Progress'
  if (status === 'closed' || status === 'complete' || status === 'completed') return 'Closed'
...
function cleanActionText(value) {
  return String(value || '')
    ...
    .filter((line) => {
      ...
      return !/^(inspection|inspection_id|question_id|section_id|photo_urls?|category|created_at|updated_at|submitted_at|answer|answer_value|answer_text|inspector_id|submitter|metadata)\b/i.test(line)
    })
    .join(' ')
    .replace(/\s{2,}/g, ' ')
    .trim()
...
function buildPdfAction(row, questionMetaById, inspectionRow, locationLine) {
  const questionMeta = questionMetaById.get(row.question_id)
  const title =
    cleanActionText(row.title) ||
    questionLabel(questionMeta?.question, row.question_id) ||
    cleanActionText(row.category) ||
    'Action'
  const issue = cleanActionText(row.comment) || title
  const rawDetails = cleanActionText(row.description)
  const details = rawDetails && rawDetails !== issue && rawDetails !== title ? rawDetails : ''
  const photoCount = parseActionPhotoUrls(row.photo_urls).length
  return {
    title,
    issue,
    details,
    photoCount,
    location: cleanActionText(row.location) || locationLine || '',
    status: formatActionStatus(row.status),
    raisedBy: cleanActionText(inspectionRow.inspector_name) || '',
  }
}
...
export async function buildFullInspectionReportPdfPayload(sqlFn, inspectionId, inspectionRow) {
  ...
  const actionRows = await sqlFn`
    SELECT id, title, description, comment, location, category, status, question_id, photo_urls, created_at
    FROM actions
    WHERE inspection_id = ${inspectionId}
  ...
  const openActions = (actionRows.rows || []).map((row) =>
    buildPdfAction(row, questionMetaById, inspectionRow, estateBlockLine)
  )
