...
import { ESTATE_WALKABOUT_CHECKLIST_QID, isWalkaboutSatelliteAnswerId } from '@/lib/estate-walkabout-template'
import {
  looksLikePersonId,
...
/** Walkabout stores follow-up comments as sibling answer keys (`qid_comment`). */
function walkaboutSiblingComment(qid, answerByQuestionId) {
  const id = String(qid || '').trim()
  if (!id) return ''
  if (id === 'ew_it_bulk_refuse_removal') {
    const exact = String(answerByQuestionId.ew_it_bulk_refuse_exact_location || '').trim()
    const comments = String(answerByQuestionId.ew_it_bulk_refuse_comments || '').trim()
    return [exact ? `Exact location: ${exact}` : '', comments].filter(Boolean).join('\n')
  }
  if (id.startsWith('ew_it_')) {
    return String(answerByQuestionId[`${id}_comment`] || '').trim()
  }
  return ''
}

/** Drop placeholder action comments that are just the Yes/No echo. */
function usefulInvestigationComment(raw) {
  const s = String(raw || '').trim()
  if (!s) return ''
  if (/^(Yes|No|NA|N\/A)$/i.test(s)) return ''
  if (/^Response:\s*(Yes|No|NA|N\/A)\s*$/i.test(s)) return ''
  if (/^Issue identified$/i.test(s)) return ''
  return s
}

function resolveFindingsComment(qid, notesComment, answerByQuestionId, isWalkaboutReport, actionCommentByQid) {
  const fromNotes = String(notesComment || '').trim()
  if (fromNotes) return fromNotes
  if (!isWalkaboutReport) return ''
  const sibling = walkaboutSiblingComment(qid, answerByQuestionId)
  if (sibling) return sibling
  return usefulInvestigationComment(actionCommentByQid?.get?.(qid) || '')
}
...
export async function buildFullInspectionReportPdfPayload(sqlFn, inspectionId, inspectionRow) {
  ...
  const actionCommentByQid = new Map()
  for (const row of actionRows.rows || []) {
    ...
    const fromComment = usefulInvestigationComment(row.comment)
    const fromDescription = usefulInvestigationComment(extractUsefulDescription(row))
    const recovered = fromComment || fromDescription
    if (recovered && !actionCommentByQid.has(qid)) {
      actionCommentByQid.set(qid, recovered)
    }
  }

  const openActionsRaw = (actionRows.rows || [])
    .map((row) =>
      buildPdfAction(
        row,
        questionMetaById,
  ...
  // Walkabout: Issues Raised is redundant with findings — keep only genuine leftovers
  // for payload consumers; PDF builder applies the same filter.
  let openActions = openActionsRaw
  // openActions may be re-filtered after sections are built (see below)

  const reportableIssueQuestionIds = new Set(
    openActionsRaw.map((action) => action.questionId).filter(Boolean)
  )
  ...
  for (const section of sections) {
    ...
    for (const question of section.questions || []) {
      ...
      if (isWalkaboutReport && isWalkaboutSatelliteAnswerId(qid)) {
      ...
      const comment = resolveFindingsComment(
        qid,
        notesComment,
        answerByQuestionId,
        isWalkaboutReport,
  ...
  if (isWalkaboutReport) {
    openActions = filterWalkaboutIssuesAlreadyInFindings(openActionsRaw, pdfSections)
  } else {
    openActions = openActionsRaw
  }
 ...
 * @param {import('@vercel/postgres').Sql} sqlFn
 * @param {{ inspectionId: string, forceRegenerate?: boolean, includeBytes?: boolean }} opts
 */
export async function ensureFullInspectionPdf(sqlFn, opts) {
  const { inspectionId, forceRegenerate = false, includeBytes = false } = opts
  ...
  if (existing && !forceRegenerate) {
    ...
    const cachedBytes = await readPdfBytesFromUrl(existing)
    if (cachedBytes) {
      return { ok: true, url: existing, generated: false, bytes: cachedBytes }
    }
    console.warn('[PDF] Existing URL unreadable for %s; regenerating', inspectionId)
  }
  ...
  try {
    ...
    return {
      ...
      url: fullPdfUrl,
      generated: true,
      ...(includeBytes ? { bytes: fullPdfBytes } : {}),
    }
  } catch (e) {
...
async function readPdfBytesFromUrl(url) {
  try {
