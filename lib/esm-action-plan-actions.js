...
function isEsmGradedQuestion(question) {
  ...
  return typeText.includes('grad')
}

const ESM_GRADED_DEFAULT_TRIGGERS = ['c', 'd']

/**
 * Whether this ESM answer should create an action.
 * Photos are never a create trigger; they are attached as evidence when an action is created.
 * Graded questions use `triggers_issue_answer` (default C,D). Yes/No questions create only when
 * they are explicitly configured as issue questions and the answer is Yes.
 */
export function shouldCreateEsmInspectionAction(question, answer) {
  if (!question) return false
  if (question.esm_hidden || question.nv_hidden) return false

  if (isEsmGradedQuestion(question)) {
    const tok = normalizeGradeAnswerToken(answer)
    if (!tok) return false
    const triggers = parseTriggersIssueAnswerList(question)
    const allowed = triggers && triggers.length > 0 ? triggers : ESM_GRADED_DEFAULT_TRIGGERS
    return allowed.includes(tok)
  }

  if (!normalizeEsmYesAnswer(answer)) return false
  return isEsmQuestionActionConfigured(question)
}
...
function getEsmStructuredExtras(extras = {}) {
  return extras?.structured && typeof extras.structured === 'object' ? extras.structured : extras
}

function getAbandonedVehicleSummary(question, extras = {}) {
  if (!question?.esm_q4_abandoned_vehicle) return []
  const structured = getEsmStructuredExtras(extras)
  const colour = typeof structured.vehicle_colour === 'string' ? structured.vehicle_colour.trim() : ''
  const makeModel = typeof structured.vehicle_make_model === 'string' ? structured.vehicle_make_model.trim() : ''
  const registration = typeof structured.vehicle_registration === 'string' ? structured.vehicle_registration.trim() : ''
  const authorisationName = typeof structured.authorisation_name === 'string' ? structured.authorisation_name.trim() : ''
  return [
    colour ? `Vehicle colour: ${colour}` : null,
    makeModel ? `Vehicle make/model: ${makeModel}` : null,
    registration ? `Vehicle registration: ${registration}` : null,
    authorisationName ? `Authorised/reported by: ${authorisationName}` : null,
  ].filter(Boolean)
}
...
function buildEsmActionDescription({ inspectionId, submittedAt, locationLine, sectionName, questionText, answer, comment, extraDetails = [], photoUrls, emailRouting, recipient, category, inspectorName }) {
  return [
...
async function createEsmActions(sql, opts) {
  ...
  for (const section of template.sections || []) {
    ...
    for (const question of section.questions || []) {
      ...
      const shouldCreate = shouldCreateEsmInspectionAction(question, answer)
      ...
      if (!shouldCreate) {
        esmLog('action skipped', {
          inspectionId,
          questionId,
          reason: 'answer_does_not_trigger_issue',
        })
        continue
      }
      ...
      esmLog(isYesFinding ? 'Yes finding recorded' : 'graded issue finding recorded', finding)
      ...
      try {
        ...
        const extraDetails = getAbandonedVehicleSummary(question, extras)
        const description = buildEsmActionDescription({
          ...
          sectionName,
          questionText,
          answer: isYesFinding ? answer : String(answer ?? ''),
          comment,
          extraDetails,
          photoUrls,
          emailRouting,
