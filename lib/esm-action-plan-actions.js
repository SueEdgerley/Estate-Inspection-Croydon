...
function isEsmPhotoActionFinding(question, photoUrls) {
  if (!isEsmGradedQuestion(question)) return false
...
async function createEsmActions(sql, opts) {
  ...
  for (const section of template.sections || []) {
    ...
    for (const question of section.questions || []) {
      ...
      const isPhotoActionFinding = isEsmPhotoActionFinding(question, photoUrls)
      ...
      esmLog('ESM response processed', {
        ...
        photoActionFinding: isPhotoActionFinding,
      })

      if (!isYesFinding && !isPhotoActionFinding) {
        esmLog('action skipped', {
          inspectionId,
          questionId,
          reason: 'answer_not_yes_or_graded_photo_finding',
        })
        continue
      }
      ...
      const finding = {
        ...
        sectionName,
        questionText,
        answer: isYesFinding ? 'Yes' : String(answer ?? ''),
        comment: comment || null,
        photo_urls: photoUrls,
        category,
      }
      findings.push(finding)
      esmLog(isYesFinding ? 'Yes finding recorded' : 'graded photo finding recorded', finding)
      ...
      if (!actionConfigured && !isPhotoActionFinding) {
        esmLog('action skipped', {
          inspectionId,
          questionId,
