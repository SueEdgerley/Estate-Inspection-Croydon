...
function buildNvExactQuestion(sectionNumber, text, questionNumber, kind = 'long_text') {
  ...
  if (kind === 'photo') {
    return {
      ...base,
      question_text: 'Add photo',
      resident_wording: 'Add photo',
      label: 'Add photo',
      question_type: 'photo',
      question_type_raw: 'photo',
      is_required: false,
      max_uploads: 1,
      include_photo: true,
      type_includes_photo: true,
        ...
        : section.grade
          ...
          : section.questions.map((question, questionIndex) =>
              buildNvExactQuestion(
                ...
                question,
                questionIndex + 1,
                sectionNumber === 24 && questionIndex === section.questions.length - 1 ? 'photo' : 'long_text'
              )
            )
...
export function isEstateFeedbackSection(sec) {
  const t = `${sec.title || ''} ${sec.name || ''}`.toLowerCase()
