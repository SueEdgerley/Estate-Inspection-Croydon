...
async function loadLogoImage(pdfDoc) {
  try {
    const pngLogoPath = path.join(process.cwd(), 'public', 'croydon-housing-logo.png')
    const logoPath = fs.existsSync(pngLogoPath)
      ? pngLogoPath
      : path.join(process.cwd(), 'public', 'croydon-housing-logo.svg')
