...
function drawPageHeader(ctx, { continuation = false } = {}) {
  ...
  page.drawRectangle({
    x: 16,
    y: headerBottom + (continuation ? 11 : 14),
    width: 154,
    height: continuation ? 40 : 46,
    color: rgb(1, 1, 1),
    borderRadius: 4,
  })
  drawLogo(
    ctx,
    22,
    headerTop - (continuation ? 12 : 14),
    PDF_LOGO_MAX_WIDTH,
    continuation ? 34 : PDF_LOGO_MAX_HEIGHT
  )
  const titleX = 186
  page.drawText('WALKABOUT RESIDENT POSTER', {
    x: titleX,
    y: headerTop - (continuation ? 28 : 30),
    size: continuation ? 12.5 : 14.5,
    font: fonts.bold,
    color: rgb(1, 1, 1),
  })
  page.drawText('Keeping your estate safe, clean and well maintained.', {
    x: titleX,
    y: headerTop - (continuation ? 44 : 48),
    size: 8,
    font: fonts.regular,
    color: rgb(1, 1, 1),
  })
