...
function buildIssueText(item) {
  const base = item.comment || item.description || item.title || item.question || 'Walkabout issue raised.'
...
function drawPageHeader(ctx) {
  ...
  const logoWidth = drawLogo(ctx, MARGIN, ctx.y, 150, 46)
...
async function drawPosterRow(ctx, item) {
  ...
  await drawPhotoCell(ctx, item, MARGIN + 10, top - 12, PHOTO_COL_WIDTH - 20, ROW_HEIGHT - 24)

  let detailY = top - 18
  detailY = drawDetailLabel(ctx, 'ISSUE', buildIssueText(item), detailsX + 12, detailY, detailsWidth - 24, 5)
  detailY = drawDetailLabel(ctx, 'JOB NUMBER', item.job_number || 'To be confirmed', detailsX + 12, detailY, detailsWidth - 24, 1)
  detailY = drawDetailLabel(ctx, 'EXPECTED COMPLETION DATE', formatDate(item.expected_completion_date), detailsX + 12, detailY, detailsWidth - 24, 1)
  drawDetailLabel(ctx, 'STATUS', formatStatus(item.status), detailsX + 12, detailY, detailsWidth - 24, 1)
