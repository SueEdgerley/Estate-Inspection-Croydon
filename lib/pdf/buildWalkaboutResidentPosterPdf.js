...
function drawLogo(ctx, x, y, maxWidth, maxHeight) {
  ...
  if (!logoImage) {
    ...
    page.drawText('Resident issues update', {
      x,
      y: y - 28,
      size: 8,
...
function drawPageHeader(ctx) {
  ...
  const logoWidth = drawLogo(ctx, MARGIN + 18, headerTop - 10, 148, 34)
  const titleX = MARGIN + Math.max(logoWidth + 36, 190)
  ...
  page.drawText('Resident Poster PDF', {
    x: titleX,
    y: headerTop - 42,
    size: 11,
    font: fonts.regular,
...
function drawPlaceholderPhoto(ctx, x, y, width, height) {
  const { page, fonts } = ctx
  page.drawRectangle({ x, y: y - height, width, height, color: hexToRgb(PHOTO_BG), borderColor: hexToRgb(BORDER), borderWidth: 1.2 })
...
function rowHeightForItem(ctx, item, detailsWidth) {
  ...
  const updateLines = measureDetailLines(ctx, updateTextForItem(item) || 'To be confirmed', detailsWidth - 24, 2)
  const locationLines = measureDetailLines(ctx, item.location || 'To be confirmed', detailsWidth - 24, 2)
  const contentHeight = 18 + issueLines * DETAIL_LINE_HEIGHT + 5 + updateLines * DETAIL_LINE_HEIGHT + 5 + locationLines * DETAIL_LINE_HEIGHT + 58
...
async function drawPosterRow(ctx, item) {
  ...
  detailY = drawDetailLabel(ctx, 'REFERENCE/JOB NUMBER', item.job_number || 'To be confirmed', detailsX + 10, detailY, detailsWidth - 20, 1)
