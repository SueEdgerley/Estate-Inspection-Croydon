...
function photoUrlsForItem(item) {
  return normalizeActionPhotoUrls(item.photo_urls)
}
...
function drawPageHeader(ctx) {
  ...
  const logoWidth = drawLogo(ctx, MARGIN + 18, headerTop - 16, PDF_LOGO_MAX_WIDTH, PDF_LOGO_MAX_HEIGHT)
...
async function drawPosterRow(ctx, item) {
  ...
  // Read the page after any break so the row frame is not drawn on the previous page.
  const { page } = ctx
  const top = ctx.y
