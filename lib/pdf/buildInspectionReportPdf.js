  ...
  if (reportableActions.length === 0) {
    ...
    for (let i = 0; i < reportableActions.length; i += 1) {
      ...
      if (newIssuePhotos.length > 0) {
        ctx.page.drawText(newIssuePhotos.length === 1 ? 'Photo' : `Photos (${newIssuePhotos.length})`, {
          x: MARGIN,
          y: ctx.y,
          size: 9,
          font: fonts.regular,
        ...
        // Wrap issue photos across the content width (consistent size, small gap).
        const gap = PHOTO_GAP
        const cols = Math.min(3, Math.max(1, Math.floor((CONTENT_W + gap) / (PHOTO_MULTI_MIN_W + gap))))
        const thumbW = Math.floor((CONTENT_W - gap * (cols - 1)) / cols)
        const thumbH = Math.max(44, Math.min(PHOTO_BOX_H, Math.round(thumbW * 0.55)))

        let drawnAny = false
        let idx = 0
        while (idx < newIssuePhotos.length) {
          ensureSpace(ctx, thumbH + 10)
          const rowBottom = ctx.y - thumbH
          const rowCount = Math.min(cols, newIssuePhotos.length - idx)
          for (let c = 0; c < rowCount; c++) {
            const url = newIssuePhotos[idx + c]
            const ok = await drawPhotoInFrame(
              ctx,
              { url },
              MARGIN + c * (thumbW + gap),
