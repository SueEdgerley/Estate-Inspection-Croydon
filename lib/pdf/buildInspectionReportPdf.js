...
import { filterWalkaboutIssuesAlreadyInFindings } from '@/lib/pdf/inspection-report-issue-filter'
import { photoGridMetrics as computePhotoGridMetrics } from '@/lib/pdf/photo-grid-metrics'
import { drawActionPhotoStack } from '@/lib/pdf/drawActionPhotos'
...
const COL = {
  question: 0.42,
  rating: 0.24, // ~12mm wider than before for badge + description breathing room
  photo: 0.34,
}

const PHOTO_BOX_W = Math.floor(CONTENT_W * COL.photo) - 16
const PHOTO_BOX_H = 58
/** Gap between thumbs when a question has multiple photos. */
const PHOTO_GAP = 4
/** Minimum thumb width before wrapping to a single column. */
const PHOTO_MULTI_MIN_W = 72
const BADGE_D = 14
const LETTER_SIZE = 8
const LABEL_SIZE = 6.5
/** Clear space from circle bottom edge to the top of the description glyphs (ESM + caretaker/walkabout). */
const BADGE_TO_TEXT_CLEARANCE = 8
/** Approximate Helvetica capital height above the baseline. */
const LABEL_ASCENT = LABEL_SIZE * 0.78
/** Space from circle bottom to text baseline (clearance + ascent). */
const BADGE_LABEL_GAP = BADGE_TO_TEXT_CLEARANCE + LABEL_ASCENT
/** Descender allowance below the label baseline. */
const LABEL_DESCENT = LABEL_SIZE * 0.28
const RESULT_BLOCK_H = BADGE_D + BADGE_LABEL_GAP + LABEL_DESCENT
/** Extra vertical padding so the Result/Rating cell isn’t cramped. */
const RESULT_CELL_PAD_Y = 8
...
/**
 * Middle-column badge styling.
 * Walkabout: answered Yes/No/N/A always print as Yes/No/N/A (never "X Not Completed").
 * "Not Completed" is reserved for unanswered Walkabout yes/no only.
 */
function resolveOutcomeStyle(question, reportVariant = REPORT_VARIANTS.DEFAULT) {
  ...
  const isWalkabout = reportVariant === REPORT_VARIANTS.WALKABOUT

  // Walkabout hard rule: yn answers → Yes / No / N/A regardless of resultMode.
  if (isWalkabout) {
    if (yn) {
    ...
    const ynMode =
      mode === 'walkabout_unanswered' ||
      mode === 'simple_yes_no' ||
      mode === 'task_yes_no' ||
      mode === 'issue_yes_no'
    if (ynMode && !raw) {
      return { kind: 'outcome', key: 'not_completed', style: OUTCOME_STYLES.not_completed }
    }
  ...
  if (yn) {
    ...
    // Mixed / default: show Yes/No/NA clearly without inventing grades
    if (yn === 'YES') return { kind: 'outcome', key: 'yes', style: OUTCOME_STYLES.yes }
    if (yn === 'NO') return { kind: 'outcome', key: 'no', style: OUTCOME_STYLES.no }
    return { kind: 'outcome', key: 'na', style: OUTCOME_STYLES.na }
  }

  if (mode === 'walkabout_unanswered' || (mode === 'simple_yes_no' && !raw)) {
    return { kind: 'outcome', key: 'not_completed', style: OUTCOME_STYLES.not_completed }
  }
...
/** Preserve explicit newlines (Walkabout structured item details), then wrap each paragraph. */
function measureMultiline(text, font, size, maxWidth, maxLines = 8) {
  const paragraphs = String(text ?? '').split(/\r?\n/)
  const lines = []
  for (const paragraph of paragraphs) {
    if (lines.length >= maxLines) break
    const wrapped = measureLines(paragraph || ' ', font, size, maxWidth, maxLines - lines.length)
...
/**
 * Badge + description as one vertically stacked unit, centred in the rating/result cell.
 * Same geometry for ESM grades and caretaker/walkabout status outcomes.
 */
function drawBadgeAndDescription(ctx, midX, midY, mark, color, caption) {
  const blockH = RESULT_BLOCK_H
  const blockTop = midY + blockH / 2
  const badgeCy = blockTop - BADGE_D / 2
  const circleBottom = badgeCy - BADGE_D / 2
  // Baseline sits clearance + ascent below the circle so glyphs never touch the ring
  const labelBaseline = circleBottom - BADGE_LABEL_GAP

  drawOutlinedBadge(ctx, midX, badgeCy, mark, color)
...
function drawMiddleResult(ctx, cellX, cellBottom, cellWidth, cellHeight, question, reportVariant) {
  const resolved = resolveOutcomeStyle(question, reportVariant)
  const midX = cellX + cellWidth / 2
  const midY = cellBottom + cellHeight / 2

  if (resolved.kind === 'grade' && resolved.style) {
    drawBadgeAndDescription(ctx, midX, midY, resolved.grade, resolved.style.color, resolved.style.label)
    return
  }

  if (resolved.kind === 'outcome' && resolved.style) {
    drawBadgeAndDescription(
      ctx,
      midX,
      midY,
      resolved.style.mark,
      resolved.style.color,
      resolved.style.label
    )
    return
  }
 ...
 * Single photo keeps the existing large frame; multiple photos wrap in a grid
 * with consistent thumb size and a small gap (never shrink below readability).
 */
export function photoGridMetrics(photoCount, cellWidth) {
  return computePhotoGridMetrics(photoCount, cellWidth, {
    photoBoxW: PHOTO_BOX_W,
    photoBoxH: PHOTO_BOX_H,
    pad: CELL_PAD,
    gap: PHOTO_GAP,
...
/** Draw one photo fitted inside a frame (aspect ratio preserved). */
async function drawPhotoInFrame(ctx, photo, frameX, frameBottom, frameW, frameH, usedUrls) {
...
/** @deprecated Prefer drawPhotoInFrame / drawPhotosInCell — kept for unlinked + issue stacks. */
async function drawPhotoInCell(ctx, photo, cellX, cellBottom, cellWidth, cellHeight, usedUrls) {
  const frameW = Math.min(PHOTO_BOX_W, cellWidth - CELL_PAD * 2)
  const frameH = Math.min(PHOTO_BOX_H, cellHeight - CELL_PAD * 2)
  const frameX = cellX + (cellWidth - frameW) / 2
  const frameBottom = cellBottom + (cellHeight - frameH) / 2
  return drawPhotoInFrame(ctx, photo, frameX, frameBottom, frameW, frameH, usedUrls)
}

/**
 * Draw every photo for a question inside the photo column, wrapping as needed.
 * Returns how many photos were successfully embedded.
 */
async function drawPhotosInCell(ctx, photos, cellX, cellBottom, cellWidth, cellHeight, usedUrls) {
  ...
  const metrics = photoGridMetrics(list.length, cellWidth)
  const gridW =
    metrics.cols * metrics.thumbW + (metrics.cols - 1) * metrics.gap
  const gridH = metrics.height
  const originX = cellX + (cellWidth - gridW) / 2
  // Align grid to vertical centre of the cell
  const originTop = cellBottom + cellHeight - (cellHeight - gridH) / 2 - metrics.pad

  let drawn = 0
  for (let i = 0; i < list.length; i++) {
    const col = i % metrics.cols
    const row = Math.floor(i / metrics.cols)
    const frameX = originX + col * (metrics.thumbW + metrics.gap)
    const frameTop = originTop - row * (metrics.thumbH + metrics.gap)
    const frameBottom = frameTop - metrics.thumbH
    const ok = await drawPhotoInFrame(
      ctx,
      list[i],
      frameX,
      frameBottom,
...
function findingsHaveOutcomes(sections) {
  for (const section of sections || []) {
    for (const q of section.questions || []) {
      if (
        q.resultMode === 'task_yes_no' ||
        q.resultMode === 'issue_yes_no' ||
        q.resultMode === 'simple_yes_no' ||
        q.resultMode === 'walkabout_unanswered'
      ) {
        return true
      }
...
function drawOutcomeKey(ctx, variant) {
  ...
  ctx.page.drawText(
    variant === REPORT_VARIANTS.CARETAKER
      ? 'Result key'
      : variant === REPORT_VARIANTS.WALKABOUT
        ? 'Answer key'
        : 'Outcome key',
    {
    x: MARGIN + 8,
...
export async function buildInspectionReportPdf(data) {
  ...
  const isWalkabout = reportVariant === REPORT_VARIANTS.WALKABOUT
  // Walkabout: Issues Raised duplicates findings (checklist + Additional items).
  // Keep only actions not already rendered as findings rows; omit section if empty.
  const reportableActions = isWalkabout
    ? filterWalkaboutIssuesAlreadyInFindings(actions, sections)
    : (actions || []).filter((action) => action.isReportableIssue !== false)
  const hasFindings = sections.some((section) => (section.questions || []).length > 0)
  const showIssuesRaised = !isWalkabout || reportableActions.length > 0
  ...
  if (hasFindings) {
    ...
    for (const section of sections) {
      ...
      ensureSpace(ctx, 28 + TABLE_HEADER_H + RESULT_BLOCK_H + RESULT_CELL_PAD_Y)
      ...
      for (const question of section.questions) {
        ...
        const qPhotoList = (photosByQuestionId[question.id] || []).filter((p) => {
          const url = String(p?.url || '').trim()
          return url && !usedPhotoUrls.has(url)
        })
        const hasPhoto = qPhotoList.length > 0
        const photoMetrics = photoGridMetrics(qPhotoList.length, cols.p)

        const qLines = measureMultiline(
        ...
        // Rows stay compact without photos, but always tall enough for the badge stack
        // and for every linked photo (wrapping grid — one row per question, never duplicated).
        const minForResult = RESULT_BLOCK_H + RESULT_CELL_PAD_Y
        const rowH = hasPhoto
          ? Math.max(photoMetrics.height, textH + CELL_PAD * 2, minForResult)
          : Math.max(textH + CELL_PAD * 2, minForResult)
        ...
        drawMiddleResult(ctx, MARGIN + cols.q, bottom, cols.r, rowH, question, reportVariant)

        if (hasPhoto) {
          await drawPhotosInCell(
            ctx,
            qPhotoList,
    ...
    if (reportableActions.length === 0) {
      ...
      for (let i = 0; i < reportableActions.length; i += 1) {
        ...
        if (issuePhotos.length > 0) {
          // Issues Raised must show ALL evidence photos even when the same URL
          // already appeared beside a findings row (do not share usedPhotoUrls).
          const beforeY = ctx.y
          const stack = await drawActionPhotoStack(ctx, issuePhotos, {
            x: MARGIN,
            y: ctx.y,
            width: CONTENT_W,
            usedUrls: new Set(),
            ...
            labelSize: 9,
            labelColor: BODY_COLOR,
            gap: PHOTO_GAP,
            minThumbW: PHOTO_MULTI_MIN_W,
            maxThumbH: PHOTO_BOX_H,
            loadJpeg: async (url) => {
              try {
                return await loadJpegForPdf(url)
              } catch {
                return null
              }
            },
          })
          ctx.y = typeof stack?.y === 'number' ? stack.y : beforeY
          if (!stack?.drawnAny) {
