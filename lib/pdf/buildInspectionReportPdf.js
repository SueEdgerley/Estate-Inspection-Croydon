...
/** Clear space from circle bottom edge to the top of the description glyphs. */
const BADGE_TO_TEXT_CLEARANCE = 6
/** Approximate Helvetica capital height above the baseline. */
const LABEL_ASCENT = LABEL_SIZE * 0.75
/** Space from circle bottom to text baseline (clearance + ascent). */
const BADGE_LABEL_GAP = BADGE_TO_TEXT_CLEARANCE + LABEL_ASCENT
const RESULT_BLOCK_H = BADGE_D + BADGE_LABEL_GAP + LABEL_SIZE * 0.25
...
/**
 * Badge + description as one vertically stacked unit, centred in the rating cell.
 * Description sits fully below the circle with a consistent gap (never overlapping).
 */
function drawBadgeAndDescription(ctx, midX, midY, mark, color, caption) {
  const blockTop = midY + RESULT_BLOCK_H / 2
