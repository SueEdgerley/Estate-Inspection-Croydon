...
const COLS = [
  { key: 'section', label: 'SECTION', width: 68 },
  { key: 'question', label: 'QUESTION / ITEM', width: 110 },
  { key: 'comment', label: 'COMMENT', width: 112 },
  { key: 'location', label: 'LOCATION', width: 58 },
  { key: 'status', label: 'ACTION / STATUS', width: 55 },
  { key: 'notes', label: 'NOTES / CHECK', width: 58 },
  { key: 'completion', label: 'COMPLETION DATE', width: 58 },
]
...
function drawTableRow(ctx, item, index) {
  ...
  const values = {
    ...
    comment: item.comment || item.description || (item.hasPhoto ? 'Photo uploaded' : ''),
  ...
  for (const col of COLS) {
    ...
    if (col.key === 'notes' || col.key === 'completion') {
      ctx.page.drawLine({
        start: { x: x + 6, y: bottom + 50 },
        end: { x: x + col.width - 6, y: bottom + 50 },
        thickness: 0.6,
        color: hexToRgb(BORDER),
      })
      ctx.page.drawLine({
        start: { x: x + 6, y: bottom + 24 },
        end: { x: x + col.width - 6, y: bottom + 24 },
        thickness: 0.6,
        color: hexToRgb(BORDER),
      })
    } else {
      drawWrappedText(ctx.page, safeText(values[col.key]), {
        ...
        y: top - 12,
        width: col.width - 8,
        font: col.key === 'question' ? ctx.fonts.bold : ctx.fonts.regular,
        size: 7.4,
        color: col.key === 'status' ? hexToRgb(BLUE) : hexToRgb(DARK),
        lineHeight: 9,
        maxLines: col.key === 'comment' ? 9 : 6,
      })
    }
    x += col.width
  }
