...
export const EW_IT_PRIVATE_GARDENS_MAINTAINED_QID = 'ew_it_private_gardens_maintained'
export const EW_IT_PRIVATE_GARDENS_OVERGROWN_QID = 'ew_it_private_gardens_overgrown'
...
function q(id, text, type, extra = {}) {
  ...
  if (type === 'yes_no' && id.startsWith('ew_it_') && id !== 'ew_it_bulk_refuse_removal') {
    questionDefaults.triggers_issue_answer = WALKABOUT_ISSUE_ON_YES_QIDS.has(id) ? 'yes' : 'no'
  }
    ...
    sections: [
      ...
      },
      {
        ...
        questions: [
          ...
          q('ew_st_esm_present', 'Is there an ESM present?', 'yes_no'),
      ...
      },
      {
        ...
        questions: [
          q(
            ...
            'Are the communal areas clear / free from obstruction?',
            'yes_no'
          ),
          q(
            'ew_it_mobility_scooters',
            'Are there any mobility scooters in the communal areas?',
            'yes_no'
          ),
          q(
          ...
          q(EW_IT_PRIVATE_GARDENS_OVERGROWN_QID, 'Are the private gardens overgrown?', 'yes_no', {
            is_required: false,
            depends_on_question_id: EW_IT_PRIVATE_GARDENS_MAINTAINED_QID,
            show_when_value: 'No',
          }),
          q(
            'ew_it_discarded_items_gardens',
      ...
      },
      {
        ...
        sort_order: 7,
        help_text:
          'Use this section only for issues that are not already covered by the checklist above. Actions are only created when Action Required is checked.',
        questions: [
          {
...
export function isWalkaboutPrivateGardensOvergrownVisible(answers) {
  return String(answers?.[EW_IT_PRIVATE_GARDENS_MAINTAINED_QID] || '').trim() === 'No'
}

/** Drop a hidden overgrown follow-up so it cannot persist as a finding or action. */
export function omitHiddenWalkaboutPrivateGardensOvergrown(answers) {
  if (!answers || typeof answers !== 'object') return answers
  if (isWalkaboutPrivateGardensOvergrownVisible(answers)) return answers
  const next = { ...answers }
  delete next[EW_IT_PRIVATE_GARDENS_OVERGROWN_QID]
  delete next[`${EW_IT_PRIVATE_GARDENS_OVERGROWN_QID}_comment`]
  return next
}

/**
 * Form stores investigation text as sibling answer keys (`ew_it_*_comment`,
 * bulk-refuse comments / exact location) — not as template questions.
 * These must persist as their own inspection_answers.question_id values.
 */
export function isWalkaboutSatelliteAnswerId(questionId) {
  const id = String(questionId || '').trim()
  if (!id) return false
  if (id === 'ew_it_bulk_refuse_comments') return true
  if (id === 'ew_it_bulk_refuse_exact_location') return true
  if (id === 'ew_it_comments') return true
  return id.endsWith('_comment') && id.startsWith('ew_it_')
}

/** Parent checklist QID for a satellite answer key (section lookup / PDF association). */
export function walkaboutSatelliteParentId(questionId) {
  const id = String(questionId || '').trim()
  if (id === 'ew_it_bulk_refuse_comments' || id === 'ew_it_bulk_refuse_exact_location') {
    return 'ew_it_bulk_refuse_removal'
  }
  if (id === 'ew_it_comments') return null
  if (id.endsWith('_comment') && id.startsWith('ew_it_')) {
    return id.replace(/_comment$/, '')
  }
  return null
}
