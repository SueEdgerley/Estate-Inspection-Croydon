...
import { normalizeYesNoAnswer, parseTriggersIssueAnswerList } from './issue-trigger-answer'
...
/**
 * Which answer direction creates an issue/action for this Yes/No question when
 * `triggers_issue_answer` is **not** set on the question (see `issue-trigger-answer.js`).
 * - Default "no" for normal cleaning checklist (No = issue).
 * - ASB/H&S/Fire "issue" style questions default to "yes" when not set (Yes = issue).
 * Override with `action_trigger_on` / `issue_triggers_on`, or set `triggers_issue_answer` (Yes/No/C/D, list).
 */
export function getActionTriggerOn(question, section) {
  if (!question) return 'no'
  const raw = question.action_trigger_on ?? question.issue_triggers_on
  if (raw === 'yes' || raw === 'on_yes') return 'yes'
  if (raw === 'no' || raw === 'on_no') return 'no'
  if (section && isSpecialSection(section) && isTriggerQuestion(question, section)) {
    return 'yes'
  }
  return 'no'
}
 ...
 */
export function validateCaretakerQuestion(question, answer, comment, photos, section = null) {
  const errors = {}
  ...
  /** When this answer is an issue trigger, enforce photo/comment (per-answer direction). */
  const requireComment =
    norm === 'yes'
      ? question.require_comment_on_yes === true
      : norm === 'no'
        ? question.require_comment_on_no !== false
        : false
  const requirePhoto =
    norm === 'yes'
      ? question.require_photo_on_yes === true
      : norm === 'no'
        ? question.require_photo_on_no !== false
        : false

  let isIssueAnswer = false
  if (triggers && triggers.length > 0) {
    isIssueAnswer = norm !== '' && triggers.includes(norm)
  } else {
    const dir = getActionTriggerOn(question, section)
    if (dir === 'yes') {
      isIssueAnswer = norm === 'yes' && question.create_action_on_yes !== false
    } else {
      isIssueAnswer = norm === 'no' && question.create_action_on_no !== false
    }
  }

  if (!isIssueAnswer) return errors

  if (requireComment) {
    if (!comment || comment.trim() === '') {
      const label = norm === 'yes' ? '"Yes"' : '"No"'
      errors[`${question.id}_comment`] = `Comment is required when answering ${label} for this question`
    }
  }
  ...
  if (requirePhoto) {
    if (!photos || photos.length === 0) {
      const label = norm === 'yes' ? '"Yes"' : '"No"'
      errors[`${question.id}_photos`] = `At least one photo is required when answering ${label} for this question`
    }
  }
