...
export function roleMayCreateInspectionWithTemplate(normalizedRole, clerkIsAdmin, template) {
  ...
  if (jobTitle === 'housing_team_manager') return isEstateWalkaboutTemplate(template)
  return false
}
...
export function filterTemplatesForAppRole(templates, viewer) {
  ...
  if (jobTitle === 'housing_team_manager') {
    const result = list.filter((t) => isEstateWalkaboutTemplate(t))
    console.log('[filterTemplatesForAppRole] housing_team_manager filter result:', result.length, 'templates')
    return result
  }
  if (jobTitle === 'esm') {
