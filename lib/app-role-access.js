...
export function normalizeJobTitle(raw) {
  ...
  if (r === 'resident_involvement_officer' || r === 'resident_involvement_officers') {
    return 'resident_involvement_officer'
  }
  if (r === 'housing_team_manager' || r === 'housing_team_managers') return 'housing_team_manager'
...
export function roleBypassesOperationalRouteRestrictions(roleCtx) {
  return isPrivilegedAdmin(roleCtx?.systemRole || roleCtx?.normalized, roleCtx?.clerkIsAdmin)
}
...
/** Job titles that may use the Estate Walkabout form (same visibility + create rules). */
export function jobTitleMayUseEstateWalkabout(jobTitle) {
  const title = normalizeJobTitle(jobTitle)
  return (
    title === 'housing_officer' ||
    title === 'housing_team_manager' ||
    title === 'resident_involvement_officer'
  )
}
...
export function roleMayCreateInspectionWithTemplate(normalizedRole, clerkIsAdmin, template) {
  ...
  if (jobTitleMayUseEstateWalkabout(jobTitle)) return isEstateWalkaboutTemplate(template)
  return false
}
...
export function filterTemplatesForAppRole(templates, viewer) {
  ...
  if (jobTitleMayUseEstateWalkabout(jobTitle)) {
    const result = list.filter((t) => isEstateWalkaboutTemplate(t))
    console.log('[filterTemplatesForAppRole] estate_walkabout filter result:', {
      job_title: jobTitle,
  ...
  // No recognised job title: do not show forms the user cannot submit.
  // (Previous DEMO FIX returned the full list and let officers fill forms that
  // the create/submit APIs would then reject with 403.)
  console.log('[filterTemplatesForAppRole] no matching job_title - returning 0 templates')
  return []
}
...
export function getRoleUiFlags(rawSystemRole, clerkIsAdmin, rawJobTitle = '') {
  ...
  if (normalizedRole === 'housing_team_manager') {
    return {
      normalizedRole,
      systemRole,
      jobTitle,
...
export function roleMayPostManualAction(normalizedRole, clerkIsAdmin) {
  ...
  return ['caretaker', 'housing_officer', 'housing_team_manager'].includes(normalizedRole)
}
