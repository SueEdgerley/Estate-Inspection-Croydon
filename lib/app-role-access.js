/**
 * Access model:
 * - users.system_role controls permissions (admin/user).
...
export function normalizeAppRole(raw) {
  ...
  if (r === 'owner' || r === 'admins' || r === 'admin') return 'admin'
  return 'user'
}
...
export async function getAppRoleContextForClerkUser(clerkUserId, clerkIsAdminFromClerk = false) {
  ...
  return {
    ...
    rawJobTitle,
    jobTitle,
    normalized: systemRole === 'admin' ? 'admin' : jobTitle,
...
export function getRoleUiFlags(rawSystemRole, clerkIsAdmin, rawJobTitle = '') {
  const systemRole = normalizeAppRole(rawSystemRole)
  const jobTitle = normalizeJobTitle(rawJobTitle)
  const normalizedRole = systemRole === 'admin' ? 'admin' : jobTitle
  if (clerkIsAdmin || systemRole === 'admin') {
    return {
      normalizedRole,
      systemRole,
      jobTitle,
