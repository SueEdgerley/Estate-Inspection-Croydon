...
import { isNeighbourhoodVoiceQuestionRenderable } from './neighbourhood-voice-template-patch.js'
...
export async function createNeighbourhoodVoiceAutoActions(sql, ctx) {
  ...
  let created = 0
  const locationLine =
    [inspection.location_label, inspection.title].filter(Boolean).join(' — ') ||
    String(inspection.title || inspection.location_label || '').trim() ||
    '—'
  const inspectionType =
    String(templateVersion?.name || inspection.template_name || 'Neighbourhood Voice').trim() ||
    'Neighbourhood Voice'
  const submittedAt = inspection.submitted_at || new Date().toISOString()
  ...
  for (const sec of sections) {
    ...
    for (const q of sec.questions || []) {
      ...
      if (q.nv_render_kind === 'nv_standard' && q._nv_issue_category) {
        ...
        `
        const nvPhotoUrls = photoRes.rows.map((r) => r.blob_url).filter(Boolean)
        const team = await formatAssignedTeamLabel(sql, recipient?.personId || null)
        await tryGenerateAndStoreIssueJobCardPdf(sql, {
          actionId,
          inspectionId,
          inspectionType,
          blockEstate: locationLine,
          location: locationLine,
          exactLocation: locationLine,
          dateRaised: formatDateGb(submittedAt),
          dateSent: formatDateGb(submittedAt),
          issueTitle: title,
          issueType: String(q._nv_issue_category || 'Neighbourhood Voice').replace(/_/g, ' '),
          issueDetail: [description, comment ? `Comments: ${comment}` : '']
            .filter(Boolean)
            .join('\n\n')
            .slice(0, 2500),
      ...
      if (q.nv_render_kind === 'nv_issues_report') {
        for (const line of NV_Q24_SUBISSUES) {
          ...
          `
          created += 1
          const photoRes2 = await sql`
            SELECT blob_url FROM inspection_photos
            WHERE inspection_id = ${inspectionId} AND question_id = ${q.id}
          `
          const nvPhotoUrls2 = photoRes2.rows.map((r) => r.blob_url).filter(Boolean)
          const team2 = await formatAssignedTeamLabel(sql, recipient?.personId || null)
          await tryGenerateAndStoreIssueJobCardPdf(sql, {
            actionId,
            inspectionId,
            inspectionType,
            ...
            dateRaised: formatDateGb(submittedAt),
            dateSent: formatDateGb(submittedAt),
            issueTitle: `${line.label} — issue`,
            issueType: String(line.issue_category || line.label || 'Issue').replace(/_/g, ' '),
            issueDetail: [detail, comment].filter(Boolean).join('\n\n').slice(0, 2500),
            priority: String(line.default_priority || 'medium'),
            assignedTeam: team2,
            targetCompletionDate: 'TBC',
            jobNumber: 'Pending assignment',
            status: 'Open',
            photoUrls: nvPhotoUrls2,
          })
        }
      }
    }
