...
export function buildAnalyticsReportPdfBuffer(data) {
  return new Promise((resolve, reject) => {
    ...
    if (ov) {
      ...
      doc.text(`Scheduled total: ${ov.scheduledTotal ?? '—'} · Completed (scheduled): ${ov.scheduledCompleted ?? '—'} · Missed: ${ov.scheduledMissed ?? '—'}`)
      doc.text(`Ad hoc (not scheduled): ${ov.adhocTotal ?? '—'} · Ad hoc submitted: ${ov.adhocCompleted ?? '—'}`)
      doc.text(`Completion rate (completed ÷ scheduled): ${ov.completionRatePct != null ? `${ov.completionRatePct}%` : '—'}`)
