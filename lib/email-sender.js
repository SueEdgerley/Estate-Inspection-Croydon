...
export async function sendEmails({ inspection, recipients, actionCategories, allActions, pdfUrl }) {
  try {
    ...
    // Send targeted emails for ASB/Health&Safety/Fire to selected recipients
    const specialCategories = ['asb', 'health_safety', 'fire_safety']
    const specialActions = allActions.filter(a => 
      specialCategories.includes(a.category) && a.recipient_person_id
    )
