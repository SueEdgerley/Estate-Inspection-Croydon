...
function patchCaretakerQuestionLabels(template) {
  ...
  for (const section of sections) {
    ...
    questions.forEach((q, index) => {
      if (!q || getCaretakerQuestionPart(q, index) !== 4) return
...
function configurePestControlQuestion(q) {
  if (!q) return q
  q.question_text = 'Is there any Pest control problems to report?'
  q.label = 'Is there any Pest control problems to report?'
  ...
  q.caretaker_recipient_options = PEST_CONTROL_RECIPIENT_OPTIONS
...
function patchCaretakerSectionStructure(template) {
  ...
  if (existingPestSection) {
    ...
    setNumberedTitle(existingPestSection, 3, 'Is there any Pest control problems to report?')
  } else {
    sections.splice(2, 0, {
      id: 'cm_pest_control_section',
      title: '3. Is there any Pest control problems to report?',
      name: '3. Is there any Pest control problems to report?',
      sort_order: 3,
