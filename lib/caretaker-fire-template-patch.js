...
function patchCaretakerQuestionLabels(template) {
  ...
  for (const section of sections) {
    ...
    questions.forEach((q, index) => {
      if (!q) return
      if (sectionNo === 1) {
        q.caretaker_photo_always = index >= 0 && index <= 3
        q.caretaker_comment_on_photo = index >= 0 && index <= 3
        q.caretaker_simple_photo_capture = index >= 0 && index <= 3
        if (index === 4) {
          q.caretaker_comment_on_yes = false
          q.caretaker_photo_on_yes = false
        }
      } else if (sectionNo === 2) {
