...
async function fetchAirtableRecords(tableName, options = {}) {
  ...
  try {
    ...
    if (options.view) {
      baseParams.append('view', options.view)
    }
    if (options.sort) {
      options.sort.forEach((sort, index) => {
        baseParams.append(`sort[${index}][field]`, sort.field)
        baseParams.append(`sort[${index}][direction]`, sort.direction || 'asc')
      })
    }
    if (options.maxRecords) {
      baseParams.append('maxRecords', options.maxRecords.toString())
    }
    ...
    do {
      ...
      if (options.maxRecords && allRecords.length >= Number(options.maxRecords)) {
        break
      }
    } while (offset)

    const finalRecords = options.maxRecords
      ? allRecords.slice(0, Number(options.maxRecords))
      : allRecords

    console.log(
      `[Airtable] Fetched ${finalRecords.length} records from ${tableName}${page > 1 ? ` across ${page} pages` : ''}`
    )
...
export async function createAirtableRecord(tableName, fields) {
  ...
  const apiKey = getAirtableKey()
  if (!baseId || !apiKey) {
    throw new Error('Airtable credentials not configured. Set AIRTABLE_BASE_ID and AIRTABLE_API_KEY (or AIRTABLE_API_TOKEN).')
  }
...
export async function getAirtableUserByClerkId(clerkUserId) {
  ...
  const raw = String(clerkUserId).trim()
  const escaped = raw.replace(/\\/g, '\\\\').replace(/"/g, '""')
  const normalized = raw.toLowerCase()
  const escapedLower = normalized.replace(/'/g, "''")
  const candidateFields = ['Users ID', 'Clerk User ID', 'User ID', 'UsersID', 'Clerk ID']
  const formulas = [
    ...candidateFields.map((field) => `{${field}} = "${escaped}"`),
    ...candidateFields.map((field) => `LOWER(TRIM({${field}} & "")) = '${escapedLower}'`),
  ]
  ...
  for (const filterByFormula of formulas) {
    try {
      ...
      const unknownField = msg.includes('unknown field') || msg.includes('invalid_filter_by_formula')
      if (unknownField) {
        console.warn('[Airtable] Users lookup formula skipped due to missing field', {
          table: TABLES.USERS,
          filterByFormula,
        })
        continue
  ...
  // Final fallback: scan Users table and match client-side across likely field names.
  try {
    const records = await fetchAirtableRecords(TABLES.USERS)
    const match = records.find((record) =>
      candidateFields.some((field) => {
        ...
        if (Array.isArray(value)) {
          return value.some((v) => String(v).trim().toLowerCase() === normalized)
        }
    ...
    console.warn('[Airtable] Users fallback scan failed', {
      table: TABLES.USERS,
...
/**
 * Fetch Users table record by exact email match (case-insensitive).
 * Returns the first matching record or null.
 */
export async function getAirtableUserByEmail(email) {
  if (!email || typeof email !== 'string' || !email.trim()) return null
  const trimmed = email.trim().toLowerCase()
  const escaped = trimmed.replace(/'/g, "''")
  const records = await fetchAirtableRecords(TABLES.USERS, {
    filterByFormula: `LOWER(TRIM({Email})) = '${escaped}'`,
    maxRecords: 1,
  })
...
/**
 * Fetch all Blocks from Airtable (table: Blocks; fields: Name, Estate).
 * Returns { id, name, estateId } sorted by name. id = Airtable record ID.
...
export async function getBlocksCached() {
  ...
  if (data.length === 0) {
    console.warn('[getBlocks] 0 blocks – check Airtable base/table/view/field names (Blocks, Name, Estate)')
  }
  return data
}
