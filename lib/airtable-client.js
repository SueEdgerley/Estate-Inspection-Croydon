...
// Normalize question type: lowercase, spaces/slashes to underscore; treat yesno as yes_no
function normalizeQuestionType(v) {
  const s = (v ?? 'text').toString().toLowerCase().trim().replace(/[\s\-/]+/g, '_').replace(/_+$/g, '') || 'text'
          ...
          const questions = (questionRecords || [])
            ...
            .map(q => {
              ...
              return {
                ...
                question_type: normalizeQuestionType(q['Question Type'] ?? q.question_type ?? 'text'),
                ...
                comment_required_when: normalizeRequiredWhen(q['Comment Required When'] ?? q.comment_required_when, q['Require Comment On No'] ?? q.require_comment_on_no),
                photo_required_when: normalizeRequiredWhen(q['Photo Required When'] ?? q.photo_required_when, q['Require Photo On No'] ?? q.require_photo_on_no),
