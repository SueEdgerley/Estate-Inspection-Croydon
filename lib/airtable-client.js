        ...
        .map(s => {
          ...
          const questions = (questionRecords || [])
            ...
            .map(q => {
              ...
              const rawType = String(q['Question Type'] ?? q.question_type ?? '').trim()
              return {
                id: q.id,
                question_text: q['Question Text'] ?? q.question_text ?? q.label ?? '',
                question_type: normalizeQuestionType(rawType || 'text'),
                type_includes_photo: rawType.toLowerCase().includes('photo'),
