...
import { applyCaretakerTemplateDisplayPatches } from './caretaker-fire-template-patch.js'
...
export async function getTemplatesNested() {
        ...
        .map(s => {
          ...
          const questions = (questionRecords || [])
            ...
            .map(q => {
              ...
              const question_key =
                questionKeyRaw != null && String(questionKeyRaw).trim() !== ''
                  ? String(questionKeyRaw).trim()
                  : q.id
              return {
                id: q.id,
                question_key,
                ...
                // Default true when unset — !!undefined is false and blocked all follow-up actions (Phase 1).
                create_action_on_no: (() => {
  ...
  return appendEstateWalkaboutTemplate(withCanonical)
}
