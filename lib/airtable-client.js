  ...
  const templates = templateRecords
    ...
    .map(t => {
      ...
      const sectionList = (sectionRecords || [])
        .filter(s => sectionRecordBelongsToTemplate(s, templateId))
        .sort((a, b) => {
          ...
          const questions = (questionRecords || [])
            .filter(
              (q) =>
                ...
                questionRawBelongsToTemplateSection(q, sectionId, Number(sortOrder) || 0)
            )
            .sort((a, b) => {
      ...
      const rawRowsMatchingTemplateSections = (questionRecords || []).filter((q) =>
        questionRawTargetsTemplateForSectionPlacement(q, templateId, sectionRecords) &&
        sectionList.some((sec) => questionRawBelongsToTemplateSection(q, sec.id, Number(sec.sort_order) || 0))
      ).length
