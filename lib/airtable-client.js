...
function getLinkedSectionIdsFromRawQuestion(q) {
  ...
  return allLinkedRecordIds(link)
}
...
export async function getTemplatesNested() {
    ...
    .map(t => {
      ...
      const rawRowsMatchingTemplateSections = (questionRecords || []).filter((q) =>
        sectionList.some((sec) =>
          questionRawBelongsToTemplateSection(q, sec.id, Number(sec.sort_order) || 0)
        )
      ).length
  ...
  const nestedOut = applyGroundsMaintenanceTemplateStructure(appendEstateWalkaboutTemplate(withCanonical))
