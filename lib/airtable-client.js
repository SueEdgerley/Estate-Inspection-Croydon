...
/**
 * Fetch all Blocks from Airtable for dropdowns.
 * Returns { id, name, estateId } sorted by name. id is the Airtable record ID.
 */
...
/**
 * getBlocks with in-memory cache (1 min) and 429 retry with backoff.
 * Use this in API routes to avoid rate limits and reduce Airtable calls.
 */
export async function getBlocksCached() {
  const now = Date.now()
  if (blocksCache && blocksCache.expiresAt > now) return blocksCache.data
