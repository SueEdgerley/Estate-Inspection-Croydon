 ...
 *
 * Selected-period management figures live on `body.management` (same object as
 * `body.directorHosReport`). A future Monthly Report button should resolve
 * This month / Previous month through resolveAnalyticsPresetDates, call
 * loadAnalyticsPayload, and render the PDF from that object only.
 */
...
import { analyticsPresetLabel, resolveIssueActionDates } from '@/lib/analytics-date-presets'
import {
...
import { gradedAbcdSql, gradedAnswerScoreSql, rankAttentionBlocks, summariseAbcd } from '@/lib/analytics-grade-summary'
import { analyticsFormBucketSql, aggregateInspectionsByForm } from '@/lib/analytics-form-labels'
import { aggregateIssueThemes } from '@/lib/analytics-issue-themes'
import { buildManagementReport } from '@/lib/analytics-management-report'
...
const ANALYTICS_ROLE_OPTIONS = [
  ...
  { value: 'housing_team_manager', label: 'Housing Team Managers', workType: 'housing_walkabout' },
]
...
export function buildActionFilterSql(whereCompletedText, whereCompletedParams, issueDateFrom, issueDateTo, issueCategory) {
  let w = `a.inspection_id IN (SELECT id FROM inspections WHERE ${whereCompletedText}) AND ${sqlExcludeFalseAutoActions('a')}`
...
export async function loadAnalyticsPayload({ searchParams, admin, internalUser }) {
  const { eff, filterAsAdmin, selectedRole, selectedPerson, presetDates } = prepareAnalyticsEffectiveParams(
    searchParams,
    admin
  )
  ...
  const blocksInspected = overviewResult.rows[0]?.blocks_inspected ?? 0

  const gradedSql = gradedAbcdSql('ia')
  const gradeSummaryResult = await run(
    `SELECT
  ...
  const grades = summariseAbcd(gradeSummaryResult.rows[0] || {})

  const inspectionsByFormResult = await run(
    `SELECT ${analyticsFormBucketSql('i')} AS form, COUNT(*)::int AS inspections
      FROM inspections i
      WHERE ${whereCompletedInspectionAlias}
      GROUP BY 1
  ...
  const inspectionsByForm = aggregateInspectionsByForm(inspectionsByFormResult.rows)

  const attentionSourceResult = await run(
    `SELECT
        COALESCE(b.id::text, '') AS block_id,
        COALESCE(NULLIF(trim(b.name), ''), 'Unknown block') AS block_name,
        COALESCE(NULLIF(trim(e.name), ''), '') AS estate_name,
        COUNT(*) FILTER (WHERE ${gradedSql} = 'A')::int AS a,
        COUNT(*) FILTER (WHERE ${gradedSql} = 'B')::int AS b,
        COUNT(*) FILTER (WHERE ${gradedSql} = 'C')::int AS c,
        COUNT(*) FILTER (WHERE ${gradedSql} = 'D')::int AS d,
        COUNT(*) FILTER (WHERE ${gradedSql} IN ('C','D'))::int AS cd,
        COUNT(*) FILTER (WHERE ${gradedSql} IN ('A','B'))::int AS ab,
  ...
  const attentionBlocks = rankAttentionBlocks(attentionSourceResult.rows)
  const scoreSql = gradedAnswerScoreSql('ia')
  let blockAverageRows = []
  try {
    const blockAverageResult = await run(
      `WITH inspection_scores AS (
          SELECT
            i.id,
            i.block_id,
            i.submitted_at,
            AVG(${scoreSql}) AS inspection_score
          FROM inspection_answers ia
          INNER JOIN inspections i ON i.id = ia.inspection_id
          WHERE ${whereCompletedInspectionAlias}
            AND lower(trim(COALESCE(ia.question_type, ''))) = 'graded'
            AND i.block_id IS NOT NULL
          GROUP BY i.id, i.block_id, i.submitted_at
          HAVING COUNT(*) FILTER (WHERE ${scoreSql} IS NOT NULL) > 0
        )
        SELECT
          COALESCE(ins.block_id::text, '') AS block_id,
          COALESCE(NULLIF(trim(b.name), ''), 'Unknown block') AS block_name,
          COUNT(*)::int AS graded_inspections,
          AVG(ins.inspection_score) AS avg_score,
          (ARRAY_AGG(ins.inspection_score ORDER BY ins.submitted_at DESC NULLS LAST, ins.id DESC))[1] AS latest_score,
          (ARRAY_AGG(ins.submitted_at ORDER BY ins.submitted_at DESC NULLS LAST, ins.id DESC))[1] AS latest_submitted_at
        FROM inspection_scores ins
        LEFT JOIN blocks b ON b.id = ins.block_id
        GROUP BY ins.block_id, b.id, b.name
        ORDER BY AVG(ins.inspection_score) ASC NULLS LAST, COUNT(*) DESC, block_name`,
      whereCompletedParams
    )
    blockAverageRows = blockAverageResult.rows || []
  } catch (e) {
    console.warn('[analytics-payload] block average query failed:', e?.message)
    blockAverageRows = []
  }
  const attentionBlocksByCount = [...attentionSourceResult.rows]
    .map((row) => ({
      ...
      d: Number(row.d) || 0,
      cd: Number(row.cd) || 0,
      gradedAbcd: Number(row.graded_abcd) || 0,
    }))
    .sort((a, b) => b.cd - a.cd || b.d - a.d || b.gradedAbcd - a.gradedAbcd)
    .slice(0, 3)
  ...
  if (hasActions) {
    ...
    issueCategories = aggregateIssueThemes(issueThemeSource).slice(0, 15)
       ...
       FROM (
         SELECT
           ...
           CASE
             ...
             WHEN lower(trim(COALESCE(p.job_title, p.role, ''))) IN ('housing team manager', 'housing team managers') THEN 'housing_team_manager'
             ELSE 'other'
           END AS role,
           COALESCE(NULLIF(trim(p.job_title), ''), NULLIF(trim(p.role), '')) AS role_label,
  ...
  try {
    const bR = await run(
      `SELECT id, name FROM blocks WHERE active IS DISTINCT FROM FALSE ORDER BY LOWER(name), name LIMIT 400`,
      []
    )
  ...
  const management = buildManagementReport({
    period: {
      ...
      dateFrom: eff.get('dateFrom') || '',
      dateTo: eff.get('dateTo') || '',
      label: analyticsPresetLabel(presetDates.preset),
    },
    inspectionsCompleted: submitted,
    blocksInspected,
    grades,
    topIssues: issueCategories,
    attentionBlocks,
    blockAverages: blockAverageRows,
    inspectionsByForm,
  })
  management.attentionBlocksByCount = attentionBlocksByCount
  ...
  const body = {
    ...
    /**
     * Canonical selected-period figures. Future Director/HOS Monthly Report PDF
     * must consume this object (also exposed as directorHosReport). Do not recompute.
     */
    management,
    directorHosReport: management,
