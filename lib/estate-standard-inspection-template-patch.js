/**
 * Estate inspection form only: selective graded + comment + photo by 1-based question order
 * (sections in order, questions within each section by sort_order).
...
/** 1-based indices (flattened form order) that get A–D–NA + comment + photo. */
const ESTATE_GRADING_PHOTO_INDICES = new Set([3, 7, 11, 14, 18, 22, 24, 25, 28, 29, 30])

// Indices 8, 13, 17, 26, 27: extras removed via the `else` branch (restore Airtable shape).
...
export function applyEstateStandardInspectionGradingPatch(template) {
  ...
  for (let i = 0; i < flat.length; i++) {
    ...
    if (ESTATE_GRADING_PHOTO_INDICES.has(n)) {
      upgradeToGradedCondition(q)
    } else {
      // All non-listed questions (incl. 8, 13, 17, 26, 27): no forced grading extras
