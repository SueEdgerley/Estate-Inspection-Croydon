 ...
 *
 * Sections whose **Section Order** (`sort_order` / `order` / `section_order`) matches
 * {@link ESTATE_SECTION_ORDER_NO_FORCED_GRADING} keep Airtable types only (no A–D–NA patch).
 *
...
import { questionLinkedSectionId } from '@/lib/inspection-template-render-sections'

/** Airtable “Section Order” for the block that must not be coerced to graded (e.g. cost codes / selects). */
export const ESTATE_SECTION_ORDER_NO_FORCED_GRADING = 4
...
/** Matches Airtable section ordering (`Section Order` → `sort_order` on nested sections). */
export function estateTemplateSectionDisplayOrder(sec) {
  const n = Number(sec?.section_order ?? sec?.sort_order ?? sec?.order ?? 0)
  return Number.isFinite(n) ? n : 0
}

/** This section follows Airtable question types only — no forced estate A–D–NA grading. */
export function isEstateSectionWithoutForcedGrading(sec) {
  return estateTemplateSectionDisplayOrder(sec) === ESTATE_SECTION_ORDER_NO_FORCED_GRADING
}

/**
 * Main “Is there an abandoned vehicle?” style Y/N (issue on **Yes**), not AVS list/cost/authorisation rows.
 */
export function isEstateAbandonedVehicleYesNoIssueRow(q, section) {
  if (!q || !section) return false
  const st = sectionTitleBlob(section)
  if (!st.includes('abandon') || !st.includes('vehicle')) return false
  if (isEstateAbandonedVehiclesNonGradedDataField(q, section)) return false
  const b = questionTextBlob(q)
  if (/\bcost\s*code\b/.test(b) || b.includes('costcode') || b.includes('cost_code')) return false
  const t = inferNormalizedQuestionType(q)
  if (t !== 'yes_no') return false
  return /\babandon/.test(b) && /\bvehicle/.test(b)
}
...
/** Cost code pickers (any section) stay as Airtable select/text — not coerced to A–D–NA. */
function isEstateCostCodePickerQuestion(q) {
  if (!q) return false
  const b = questionTextBlob(q)
  if (!/\bcost\s*code\b/.test(b) && !b.includes('costcode') && !b.includes('cost_code')) return false
  const raw = String(q.question_type_raw ?? '').toLowerCase()
  if (raw.includes('select')) return true
...
export function isEstateInspectionChecklistBodyRow(q, sec) {
  ...
  if (isEstateSectionWithoutForcedGrading(sec)) return false
  ...
  if (isEstateCostCodePickerQuestion(q)) return false
  const raw = String(q.question_type_raw ?? '').toLowerCase()
...
function applyEstateRowGradingDecision(q, sec) {
  if (sec && isEstateSectionWithoutForcedGrading(sec)) {
    restoreQuestionToAirtableShape(q)
    return
  }
  if (isEstateAbandonedVehicleYesNoIssueRow(q, sec)) {
    restoreQuestionToAirtableShape(q)
    q.question_type = 'yes_no'
    q.answer_mode = 'yes_no'
    if (q.triggers_issue_answer == null || q.triggers_issue_answer === '') {
      q.triggers_issue_answer = 'Yes'
    }
    q.create_action_on_no = false
    q.require_comment_on_no = false
    q.require_photo_on_no = false
    q.create_action_on_yes = true
    if (q.require_comment_on_yes === undefined) q.require_comment_on_yes = true
    if (q.require_photo_on_yes === undefined) q.require_photo_on_yes = true
    return
  }
  const avDataOnly = isEstateAbandonedVehiclesNonGradedDataField(q, sec)
  const costCodeRow = isEstateCostCodePickerQuestion(q)
  if (shouldPreserveNonGradedShape(q, sec) || avDataOnly || costCodeRow) {
    restoreQuestionToAirtableShape(q)
    if (avDataOnly) {
      q.comment_required_when = null
      q.photo_required_when = null
