 ...
 */
/**
 * Numbered checklist rows only (Q1, Q2, Q3…): skips instructions, routing, NV, Q12 AV data fields.
 * Same ordering as the estate inspection form preview.
 * @param {Record<string, unknown>} template
 * @returns {Map<string, number>} question id → 0-based index (Q1 = 0, Q2 = 1, Q3 = 2)
 */
...
export function applyEstateStandardInspectionGradingPatch(template) {
  ...
  /** Snapshot Airtable display flags before upgrade (used to mirror Q2 → Q3). */
  const preFlags = new Map()
  for (const sec of sortedSections(sections)) {
    for (const q of sortedQuestions(sec.questions)) {
      preFlags.set(q.id, {
        caretaker_graded_always_extras: !!q.caretaker_graded_always_extras,
        standard_inspection_condition_row: !!q.standard_inspection_condition_row,
        nv_graded_require_comment_photo: !!q.nv_graded_require_comment_photo,
        nv_graded_require_comment_only: !!q.nv_graded_require_comment_only,
      })
    }
  }
  ...
  /** Q3 (third checklist row) must mirror Q2: same graded + comment/photo flags as the second checklist row after Airtable upgrade. */
  const checklist = []
  for (const sec of sortedSections(sections)) {
    for (const q of sortedQuestions(sec.questions)) {
      if (!isEstateInspectionChecklistBodyRow(q, sec)) continue
  ...
  if (checklist.length >= 3) {
    const q2 = checklist[1]
    const q3 = checklist[2]
    const s2 = preFlags.get(q2.id)
    upgradeEstateInspectionQuestionToGraded(q3)
    if (s2) {
      if (s2.caretaker_graded_always_extras) q3.caretaker_graded_always_extras = true
      if (s2.standard_inspection_condition_row) q3.standard_inspection_condition_row = true
      if (s2.nv_graded_require_comment_photo) q3.nv_graded_require_comment_photo = true
      if (s2.nv_graded_require_comment_only) q3.nv_graded_require_comment_only = true
    }
  }
