...
export async function handleNoAnswer({
  ...
  try {
    ...
    const existingResponse = await fetch(`/api/actions?inspection_id=${inspectionId}&question_id=${questionId}&status=all&include_false=1`, { credentials: 'include' })
    ...
    const actionData = {
      ...
      description:
        richDescription ||
        `Section: ${sectionName} – ${questionText}`,
...
export async function handleYesAnswer(inspectionId, questionId) {
  try {
    ...
    const response = await fetch(`/api/actions?inspection_id=${inspectionId}&question_id=${questionId}&status=all&include_false=1`, { credentials: 'include' })
