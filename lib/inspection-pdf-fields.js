...
export function withInspectionPdfDefaults(row) {
  ...
  return {
    ...row,
    full_pdf_url: row.full_pdf_url ?? row.fullPdfUrl ?? null,
    pdf_url: row.pdf_url ?? row.pdfUrl ?? null,
    poster_pdf_url: row.poster_pdf_url ?? row.posterPdfUrl ?? null,
    pdf_generation_error: row.pdf_generation_error ?? row.pdfGenerationError ?? null,
  }
}
